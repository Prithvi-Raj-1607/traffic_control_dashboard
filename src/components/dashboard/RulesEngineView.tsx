  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        <div className="flex items-center gap-2 mb-4"><Filter className="w-4 h-4 text-[#66B800]" /><h3 className="text-sm font-bold text-gray-900">Rule Filters</h3></div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            <label className="text-xs text-gray-500 mb-1 block">Min Support: {minSupport.toFixed(2)}</label>
            <input type="range" min="0.01" max="0.3" step="0.01" value={minSupport} onChange={(e) => setMinSupport(parseFloat(e.target.value))} className="w-full accent-[#66B800]" />
          </div>
          <div>
            <label className="text-xs text-gray-500 mb-1 block">Min Confidence: {minConfidence.toFixed(2)}</label>
            <input type="range" min="0.1" max="0.9" step="0.05" value={minConfidence} onChange={(e) => setMinConfidence(parseFloat(e.target.value))} className="w-full accent-[#66B800]" />
          </div>
          <div>
            <label className="text-xs text-gray-500 mb-1 block">Min Lift: {minLift.toFixed(1)}</label>
            <input type="range" min="1.0" max="4.0" step="0.1" value={minLift} onChange={(e) => setMinLift(parseFloat(e.target.value))} className="w-full accent-[#66B800]" />
          </div>
        </div>
      </div>
          ...
          <div key={rule.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-3 mb-3">
              <div className="flex items-center gap-2 flex-1 flex-wrap">
                <span className="px-2.5 py-1 bg-[#66B800]/10 text-[#66B800] text-xs font-bold rounded-lg">{rule.antecedent}</span>
                <ArrowRight className="w-4 h-4 text-gray-400 flex-shrink-0" />
                <span className="px-2.5 py-1 bg-red-50 text-red-600 text-xs font-bold rounded-lg">{rule.consequent}</span>
              </div>
            </div>
            <div className="flex items-center gap-3 mb-2 flex-wrap">
              <div className="text-xs"><span className="text-gray-400">Support: </span><span className="font-semibold text-gray-700">{(rule.support * 100).toFixed(1)}%</span></div>
              <div className="text-xs"><span className="text-gray-400">Confidence: </span><span className="font-semibold text-gray-700">{(rule.confidence * 100).toFixed(1)}%</span></div>
              <div className="text-xs"><span className={`font-bold px-1.5 py-0.5 rounded ${getLiftColor(rule.lift)}`}>Lift: {rule.lift.toFixed(2)}x</span></div>
            </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        ...
        <div className="max-h-96 overflow-y-auto custom-scrollbar">
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-white"><tr className="border-b border-gray-200"><th className="text-left py-2.5 font-semibold text-gray-500">Antecedent</th><th className="text-center py-2.5 font-semibold text-gray-500">&rarr;</th><th className="text-left py-2.5 font-semibold text-gray-500">Consequent</th><th className="text-center py-2.5 font-semibold text-gray-500">Support</th><th className="text-center py-2.5 font-semibold text-gray-500">Confidence</th><th className="text-center py-2.5 font-semibold text-gray-500">Lift</th><th className="text-left py-2.5 font-semibold text-gray-500 max-w-[200px]">Interpretation</th></tr></thead>
            <tbody>
              {filteredRules.map((rule) => (
                <tr key={rule.id} className="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  <td className="py-2.5 text-center font-bold" style={{ color: rule.lift >= 3 ? '#EF4444' : rule.lift >= 2 ? '#F59E0B' : '#6B7280' }}>{rule.lift.toFixed(2)}</td>
